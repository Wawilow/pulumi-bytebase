...
//go:embed cmd/pulumi-resource-bytebase/bridge-metadata.json
...
func convertName(tfname string) (module string, name string) {
	...
	contract.Assertf(tfNameItems[0] == "bytebase", "Invalid snake case name %s. Does not start with bytebase", tfname)
...
func makeDataSource(ds string) tokens.ModuleMember {
	...
	return tfbridge.MakeDataSource("bytebase", mod, "get"+name)
}

func makeResource(res string) tokens.Type {
	mod, name := convertName(res)
	return tfbridge.MakeResource("bytebase", mod, name)
}
